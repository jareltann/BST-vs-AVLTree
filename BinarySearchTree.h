...
*****************************************************/
    ...
    void displayLinks( BinaryNode *t, int depth, ostream & out ) const
    {
        ...
        {
            for( int i = 0 ; i < SHIFT*depth ; i++ ){ out << " " ; }
            out << t->element << ") "; out << "@:" <<  &(t->element) << "  L:" << t->left << "  R:" << t->right << endl;
	...
	int depth( BinaryNode *t, const Comparable &x, int level) const
	{

		if (t == nullptr && contains(x,t) == true){
			return -1;
		}
		else if (t == nullptr){
			return level;
